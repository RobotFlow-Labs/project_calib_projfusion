...
def build_results_dataframe(results: list[dict[str, object]]) -> pd.DataFrame:
    return pd.DataFrame(results).sort_values(["dataset_id", "perturbation_range"]).reset_index(drop=True)
...
def save_report_bundle(results: list[dict[str, object]], output_dir: str | Path) -> tuple[Path, Path]:
    ...
    markdown_path.write_text(build_table_i(results) + "\n" + build_table_ii(results), encoding="utf-8")
