...
class CalibrationService:
    ...
    def calibrate(self, request: CalibrationRequest, debug_overlay: bool = False) -> CalibrationResponse:
        ...
        point_cloud = torch.tensor(request.point_cloud, dtype=torch.float32).unsqueeze(0)
        init_extrinsic = torch.tensor(request.init_extrinsic, dtype=torch.float32).unsqueeze(0)
        camera = CameraInfo.from_mapping(request.camera_info.model_dump())
