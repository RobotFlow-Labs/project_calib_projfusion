...
from anima_calib_projfusion.geometry.projection import (
    align_point_groups,
    normalize_grid,
    project_points,
)
...
def test_project_points_maps_to_feature_plane():
    camera_info = {
        ...
        "cy": torch.tensor([25.0]),
        "sensor_h": 100,
        "sensor_w": 200,
    }
    xyz = torch.tensor([[[0.0, 0.0, 1.0], [0.5, 0.25, 1.0]]])
    uv = project_points(xyz, camera_info, feature_hw=(10, 20))
...
def test_align_point_groups_outputs_clamped_normalized_grid():
    ...
    grid = align_point_groups(xyz, extrinsic, camera_info, feature_hw=(10, 20), margin=2.0)
    uv = project_points(xyz, camera_info, feature_hw=(10, 20))
    normalized = normalize_grid(uv, feature_hw=(10, 20))
